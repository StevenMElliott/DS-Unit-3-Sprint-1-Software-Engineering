...
First Question: The most important thing I am looking for in a code review is 
readability. If we're going to collaborate, we need to be able to read the code.
It is nice to have #comments that explicitly state what each part is about. The
reason why I think comments are important is that it makes the code easy to
pick up and make adjustment to. You can also check if the comment light up with
what the code is doing.

Second Question: Containers are useful because they provide a built-in environment 
that the code is known to have worked in. The reason why this is important is 
modulesare constantly being updated. With those updates, functions may be changed 
ormerged with others which would break that bit of code. 
